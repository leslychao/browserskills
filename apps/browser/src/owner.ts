...
export interface OwnerOptions {
  ...
  startUrl?:string;profiles?:readonly TemplateProfile[];verificationTimeoutMs?:number;
}
...
export class BrowserOwner extends EventEmitter {
  ...
  status():BrowserStatus{return {workerId:this.options.workerId,generation:this.generation,mode:this.mode,url:this.context?.pages()[0]?.url()??null,runId:this.runId};}
