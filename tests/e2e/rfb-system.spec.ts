...
test('real noVNC, Spring proxy and Docker x11vnc connect; automation and logout revoke live sockets',async({page,browser})=>{
  ...
  await page.getByLabel('Заданий максимум').fill('1');const closed=socket.waitForEvent('close');await page.getByRole('button',{name:'Начать',exact:true}).click();await closed;
