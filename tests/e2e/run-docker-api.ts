/** Production image smoke: real PostgreSQL, LAN HTTP, static bundle and session lifecycle. */
...
try{
  ...
  async function request(path:string,method='GET',body?:unknown,headers:RequestOptions['headers']={}){
    return new Promise<{status:number;body:string;headers:import('node:http').IncomingHttpHeaders}>((done,reject)=>{
      ...
      const request=httpRequest({hostname:'127.0.0.1',port,path,method,timeout:10_000,headers:{...(cookie?{Cookie:cookie}:{}),...(data?{'Content-Type':'application/json','Content-Length':Buffer.byteLength(data)}:{}),...headers}},response=>{
        let text='';
      ...
      request.once('error',reject);request.once('timeout',()=>request.destroy(new Error('HTTP timeout')));request.end(data);
    });
  }
  let live=false;
  ...
  if(!live)throw new Error(`Production HTTP startup failed: ${await command('docker',['logs','--tail','80',api])}`);
